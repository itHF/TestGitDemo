...
public class UITooltipMenu : MonoBehaviour
{
	static protected UITooltipMenu mInstance;
	...
	protected virtual void Start ()
	{
		...
		SetAlpha(0f);
        slerp = background.transform.localPosition + text.transform.localPosition;
    }
	...
	protected virtual void SetText (string tooltipText)
	{
		if (text != null && !string.IsNullOrEmpty(tooltipText))
		{
			...
			if (uiCamera != null)
			{
                ...
                {
                    if (targetAnchor.GetComponent<UISprite>().pivot == UIWidget.Pivot.BottomLeft)
                        background.pivot = UIWidget.Pivot.TopLeft;
                    else if (targetAnchor.GetComponent<UISprite>().pivot == UIWidget.Pivot.BottomRight)
                        background.pivot = UIWidget.Pivot.TopRight;

                    mTrans.localPosition = targetAnchor.localPosition;
                    background.SetDimensions(text.width + 28, background.height);
	...
	static public void ShowText (string tooltipText, Transform targetObj)
	{
		if (mInstance != null)
		{
            targetAnchor = targetObj;
